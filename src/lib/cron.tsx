...
const Cron: React.FunctionComponent<CronProp>  = (props) => {
    ...
    const getHeaders = () => {
        return state.headers.map((d, index) => {
            return <li className="nav-item" key={index} ><a className={`nav-link ${state.selectedTab === d ? 'active' : ''} ${props.disabled ? "disabled" : ""}`} onClick={()=> tabChanged(d)}>{translate(d)}</a></li>
        })
    }
    ...
    const getComponent = (tab: HeaderValType) => {
        ...
        return <CronComponent translate={translate} value={state.value} onChange={onValueChange} disabled={props.disabled}/>;
    }
